    ...
    def smtp_connection(self, cursor, user, id, context=None):
        ...
        if core_obj.smtpserver and core_obj.smtpport and \
            ...
            logger.notifyChannel(
                _("Power Email"),
                netsvc.LOG_ERROR,
                _("Mail from Account %s failed. Probable Reasons: Account not approved/SMTP login failed") % id)
    ...
    def extracttime(self, time_as_string):
        ...
        try:
            ...
            logger.notifyChannel(
                    ...
                    netsvc.LOG_WARNING,
                    _(
                      "Datetime Extraction failed. Date: %s. Error: %s"
                    ) % (time_as_string, exc))
        return date_as_date
    ...
    def save_header(self, cr, uid, mail, coreaccountid, serv_ref, context=None):
        ...
        #print vals
            ...
            logger.notifyChannel(
                    _("Power Email"),
                    netsvc.LOG_ERROR,
                    _("Save Header -> Mailbox create error. Account: %s, Mail: %s, Error: %s") % (coreaccountid, serv_ref, str(e)))
                ...
                if rec.iserver_type == 'imap' and rec.isfolder:
                    ...
                    try:
                        ...
                        logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_ERROR, 
                                _("IMAP Server Error Account: %s Error: %s.") % (id, error))
                    #Try logging in to server
                    try:
                        serv.login(rec.isuser, rec.ispass)
                    except imaplib.IMAP4.error, error:
                        logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_ERROR, 
                                _("IMAP Server Login Error Account: %s. Error: %s.") % (id, error))
                    logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_INFO, 
                                _("IMAP Server Connected & logged in successfully Account: %s.") % (id))
                    #Select IMAP folder
                    try:
                        ...
                        logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_ERROR, 
                                _("IMAP Server Folder Selection Error. Account: %s. Error: %s.") % (id, error))
                    logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_INFO, 
                                _("IMAP Folder selected successfully Account: %s.") % (id))
                    logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_INFO, 
                                _("IMAP Folder Statistics for Account: %s: %s") % (
                           id, 
                           serv.status(
                                '"%s"' % rec.isfolder, 
                    ...
                    try:
                        ...
                        logger.notifyChannel(
                            _("Power Email"), 
                            netsvc.LOG_ERROR, 
                            _("POP3 Server Error. Account: %s Error: %s.") % (id, error))
                    #Try logging in to server
                    try:
                        serv.user(rec.isuser)
                        serv.pass_(rec.ispass)
                        ...
                        logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_ERROR, 
                                _("POP3 Server Login Error. Account: %s Error: %s.") % (id, error))
                    logger.notifyChannel(
                                _("Power Email"), 
                                netsvc.LOG_INFO, 
                                _("POP3 Server Connected & logged in successfully Account: %s.") % (id))
                    logger.notifyChannel(_("Power Email"), netsvc.LOG_INFO, _("POP3 Statistics: %s mails of %s size for Account: %s:") % (serv.stat()[0], serv.stat()[1], id))
                    #Download Full RF822 Mails
                    ...
                    logger.notifyChannel(
                        _("Power Email"), 
                        netsvc.LOG_ERROR, 
                        _("Incoming server login attempt dropped Account: %s. Check if Incoming server attributes are complete."
                        ) % (id))
